...
#### Регистрация
Шаблон запроcа. Пароль минимум 8, максимум 60 по длине, обязательно 1 цифра, прописная и строчная буква и 1 символ(например !@ и тд)
```bash
curl -X 'POST' \
...
```
#### Создать выражение
Шаблон запроса на создание выражения, вставьте в него выражение и токен вместо `<token>` и отправьте в `Git Bash`
```bash
curl -X 'POST' \
...
Шаблон запроса. Вставьте в него номер страницы и количество выражений на странице, токен вместо `<token>` и отправьте в `Git Bash`
```bash
curl -X 'GET' \
...
Шаблон запроса. Вставьте вместо {id} id выражения, токен вместо `<token>`  и отправьте запрос в `Git Bash`
```bash
curl -X 'GET' \
