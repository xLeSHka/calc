...
**Unary минус** нужно выделять в скобки, если он не стоит после скобки. `log(a,x)` представляет из себя `log10(a)`/`log10(x)` то есть `loga(x)`. На этом все, мне было лень добавлять больше функций :0
